...
public class InsertionSort {
	...
	public static double[] insertionSort(double[] numeros) {
		...
		for (i = 1; i < numeros.length; i++) { //iterage em todos os elementos do array.
			...
			j = i;//seto a variável j com sendo o valor contido em i

			while ((j > 0) && (numeros[j - 1] > daVez)) {//iterar enquanto j for > 0 
				//verifica se o numero anterior contido no index atual -1 é maior que o valor contido no index atual.
				//caso seja faz a troca
				numeros[j] = numeros[j - 1]; //verifico se o valor contido no index j é 
